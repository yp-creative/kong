...
local HTTP_STATUS = require 'kong.tools.responses'.status_codes
...
_M.process = function(ctx, token)
  ...
  if not res or res.status == HTTP_STATUS.HTTP_NOT_FOUND then log.error("failed to request oauth2 server: ", err) response.oauth2Exception(appKey, err) end
  local body = json.decode(res.body)
  if res.status ~= HTTP_STATUS.HTTP_OK then response.oauth2Exception(appKey, body.error) end
