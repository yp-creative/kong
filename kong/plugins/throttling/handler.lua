...
local ThrottlingHandler = BasePlugin:extend()

ThrottlingHandler.PRIORITY = 930

local function increment(api_id, identifier, current_timestamp, value)
  -- Increment metrics for all periods if the request goes through
  local _, stmt_err = dao.throttling_metrics:increment(api_id, identifier, current_timestamp, value)
  if stmt_err then
...
local function get_usage(api_id, identifier, current_timestamp, limits)
  ...
  for name, limit in pairs(limits) do
    local current_metric, err = dao.throttling_metrics:find_one(api_id, identifier, current_timestamp, name)
    if err then
      return nil, nil, err
    end
...
function ThrottlingHandler:new()
  ThrottlingHandler.super.new(self, "throttling")
end

function ThrottlingHandler:access(conf)
  ThrottlingHandler.super.access(self)
