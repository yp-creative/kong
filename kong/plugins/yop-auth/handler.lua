...
local initializeCtx = require 'kong.plugins.yop-auth.interceptor.initialize_ctx'

local security_center = require 'kong.yop.security_center'
local response, _ = require 'kong.yop.response'()
local interceptors = {
  require 'kong.plugins.yop-auth.interceptor.decrypt',
  require 'kong.plugins.yop-auth.interceptor.validate_sign'
}

local keyOrder = { "state", "result", "ts", "sign", "error" }

local YopAuthHandler = BasePlugin:extend()

function YopAuthHandler:new() end

function YopAuthHandler:access()
--  -- 不是appKey+appSecret方式鉴权,跳过该插件
--  if ngx.ctx.authMode ~= "YOP-AUTH" then return end;
...
function YopAuthHandler:body_filter()
  --  异常情况，主动跳过后续的加密签名处理
...
--  if ngx.ctx.authMode ~= "YOP-AUTH" then return end;
  ...
  ngx.log(ngx.NOTICE,"r: "..require "kong.yop.luaTableDump"(r))
  ngx.arg[1] = json.encode(r, { indent = true, keyOrder = keyOrder })
end

YopAuthHandler.PRIORITY = 800
return YopAuthHandler